  ...
  constructor() {
    ...
    this.Reports = asyncHandler(async (req, res) => {
      try {
        ...
        // Preload user test histories for all subtopics & templates in a single call
        const testHistories = await TestHistory.findAll({
          where: {
            user_id: user.id,
            sub_topic: { [Op.in]: subtopics.map((s) => s.id) },
        ...
        // Build fast lookup map: { `${subtopicId}_${templateId}`: history }
        const historyMap = {};
        for (const h of testHistories) {
          const key = `${h.sub_topic}_${h.question_type}`;
          if (!historyMap[key]) historyMap[key] = h;
        }

        // Preload all TestHistory question_ids by subtopic for SkillReport
        const testHistoryBySubtopic = await TestHistory.findAll({
          where: {
            user_id: user.id,
            sub_topic: { [Op.in]: subtopics.map((s) => s.id) },
            is_deleted: null,
          },
          attributes: ["sub_topic", "question_ids"],
          raw: true,
        });

        const subtopicQuestionMap = {};
        for (const item of testHistoryBySubtopic) {
          if (!subtopicQuestionMap[item.sub_topic]) {
            subtopicQuestionMap[item.sub_topic] = [];
          }
          if (Array.isArray(item.question_ids)) {
            subtopicQuestionMap[item.sub_topic].push(...item.question_ids);
          }
        }

        // Preload all attended test question attempts at once
        const allQuestionIds = [
          ...new Set(
            testHistoryBySubtopic.flatMap((h) =>
              Array.isArray(h.question_ids) ? h.question_ids : []
            )
          ),
        ];
        ...
        if (allQuestionIds.length > 0) {
          const attended = await AttendedTestQuestion.findAll({
            where: { question_id: { [Op.in]: allQuestionIds } },
          ...
          for (const att of attended) {
            if (!attendedMap[att.question_id])
              attendedMap[att.question_id] = [];
            attendedMap[att.question_id].push(att);
          }
        }
        ...
        const data = await Promise.all(
          subtopics.map(async (subtopic) => {
            const qids = subtopicQuestionMap[subtopic.id] || [];
            ...
            if (uniqueQids.length > 0) {
              for (const qid of uniqueQids) {
                const attempts = attendedMap[qid] || [];
                ...
                questions.push({
                  ...
                  second_last_result: convert(secondLast),
                });
              }
            }
            ...
            // Calculate percent using preloaded historyMap
            const percents = templates.map((tid) => {
              const h = historyMap[`${subtopic.id}_${tid}`];
              if (!h) return 0;
              const total = h.correct_ans + h.wrong_ans;
              return total === 0 ? 0 : (h.correct_ans / total) * 100;
            });
