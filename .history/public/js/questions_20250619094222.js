    ...
    $.ajax({
      ...
      success: async function (res) {
        if (res.status === 200) {
          const data = res.data;
          if (type === 1 || type === "1") {
            let ques_html_cho = makeHTMLforChooseUp(id, data.data);
            $(".choose_questions_container").html(ques_html_cho);
            OpenModal("question_list_edit_modal");
          } else if (type === 2 || type === "2") {
            let ques_html_mat = makeHTMLforMatchUp(id, data.data);
            $(".match_questions_container").html(ques_html_mat);
...
function makeHTMLforMatchUp(id, data) {
  let html = ``;
...
function makeHTMLforChooseUp(id, data) {
