...
function loadQuestion() {
  ...
  questionContainer.innerHTML = `
        <div class="quiz-question">
            <h2 id="question-text" onmouseover="handleQuestionHover('${
              question.question
  ...
  question.options.forEach((option, index) => {
    ...
    optionDiv.innerHTML = `
            <img src="/${option.image}" style="display:${
      option.image ? "block" : "none"
    }" alt="${option.text}" class="option-image">
...
function toggleAnswerQ0(selectedIndex) {
  ...
  if (selectedAnswers.length === 2) {
    ...
    new Audio(
      isAnyCorrect ? "/activity/admin/right.mp3" : "/activity/admin/wrong.mp3"
    ).play();
