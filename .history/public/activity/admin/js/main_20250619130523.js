...
function startReminderTimer() {
    ...
    reminderTimer = setInterval(() => {
        ...
        const isUnanswered = (currentQuestion === 0 && answer.length < 2) ||
...
optionDiv.classList.add("answer-option");
    ...
    if (userAnswers[currentQuestion] === null || (currentQuestion === 0 && userAnswers[0].length === 0)) {
...
function toggleAnswerQ0(selectedIndex) {
    ...
    if (selectedAnswers.length === 2) {
        ...
        const isFullyCorrect = selectedAnswers.every(i => correctSet.has(i)) && selectedAnswers.length === correctSet.size;
        ...
        document.querySelectorAll('.answer-option').forEach((opt, idx) => {
            if (correctSet.has(idx)) {
                ...
                if (!opt.querySelector('.result-icon')) {
                    opt.innerHTML += '<div class="result-icon"><i class="fa fa-check colour-green"></i></div>';
                }
            }
        });
        ...
        const correctText = questions[0].correct.map(i => questions[0].options[i].text).join(' and ');
...
function checkAnswer(selectedIndex) {
    ...
    if (isCorrect) {
        ...
        answerOptions[selectedIndex].innerHTML += '<div class="result-icon"><i class="fa fa-check colour-green"></i></div>';
        ...
        answerOptions[selectedIndex].innerHTML += '<div class="result-icon"><i class="fa fa-times colour-red"></i></div>';

        answerOptions.forEach((opt, idx) => {
            if (Array.isArray(question.correct) ? question.correct.includes(idx) : idx === question.correct) {
                opt.classList.add("correct");
                opt.innerHTML += '<div class="result-icon"><i class="fa fa-check colour-green"></i></div>';
            }
        });
...
function updateNavigationButtons() {
    ...
    document.getElementById("next-btn").disabled = currentQuestion === 0
...
function speakQuestionOrResult() {
    ...
    if (userAnswer === null || (currentQuestion === 0 && userAnswer.length === 0)) {
        speakQuestion(question.question);
    } else {
