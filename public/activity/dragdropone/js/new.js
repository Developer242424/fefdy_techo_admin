...
function updateCheckButton() {
  const required = ['apple', 'car', 'ball'];
  const leftMatches = new Set(lines.map(line => line.leftId));
  document.getElementById('checkBtn').disabled = !required.every(id => leftMatches.has(id));
} 
...
startDot.el.classList.add('active-match', 'active'); // already done
matchedRightDot.el.classList.add('active-match', 'active'); // 👈 add this line
