  ...
  $.ajax({
    ...
    success: function (res) {
      if (Array.isArray(res)) {
        ...
        questionIds = gameData[0].questionid;
      } else {
...
// Function to dynamically generate content based on the JSON data
  ...
  function handleStart(e, dot) {
    ...
    dot.classList.add("active-drag", "active");
    e.preventDefault();
  }
  ...
  function handleEnd(e) {
    ...
    document.querySelectorAll(".item.right").forEach((item) => {
      ...
      if (
        ...
        const dot = item.querySelector(".dot.right");
        if (dot) {
          matchedRightDot = {
            el: dot,
            id: dot.dataset.id,
  ...
  // Attach mouse and touch handlers to each left dot
  document.querySelectorAll(".dot.left").forEach((dot) => {
    dot.addEventListener("mousedown", (e) => handleStart(e, dot));
    dot.addEventListener("touchstart", (e) => handleStart(e, dot), {
      passive: false,
    });
  });
