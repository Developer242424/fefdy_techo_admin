...
//     {
//         "questionid": 1,
//         "question": "Which of these is a wild animal?",f
...
function loadQuestion() {
  ...
  questionData.options.forEach((opt, index) => {
    ...
    card.innerHTML = `
        ...
        <img class="choose-img" src="${publicURL}${opt.image}" alt="${opt.primary_text
      }" onmouseover="playSpeech('${opt.primary_text.replace(/'/g, "\\'")}')">
        <hr class="divider">
        <div class="primary-text" onmouseover="playSpeech('${opt.primary_text.replace(
        /'/g,
        "\\'"
      )}')">${opt.primary_text}</div>
        <div class="secondary-text" onmouseover="playSpeech('${opt.secondary_text.replace(
        /'/g,
        "\\'"
      )}')">${opt.secondary_text}</div>
        `;
  ...
  updateProgressBar();
  setTimeout(() => {
    playSpeech(questionData.question);
  }, 300);
}
function showRetryModal(correctCount, totalQuestions) {
  ...
  <div class="modal-overlay">
    ...
    // Show success modal directly without retry option
    modal.remove();
    showSuccessModal();
  } else {
    showSuccessModal();
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (currentQuestion < choose_questions.length) {
            ...
            stopTimer();
            updateProgressBar();
            showRetryModal(score, choose_questions.length);
          }
        }, 500);
      }, 500);
    }
...
function showSuccessModal() {
  ...
  scoreDisplay.textContent = `${final_score}`;
...
function adjustScaleForIframe() {
  ...
  if (insideIframe) {
    ...
    else if (width < 1200) scaleValue = 0.8;
    else scaleValue = 1;
...
function isInFullscreen() {
  const heightMatch = Math.abs(window.innerHeight - screen.height) < 50;
  const widthMatch = Math.abs(window.innerWidth - screen.width) < 50;
  ...
  return heightMatch && widthMatch || apiFullscreen;
}

function updateFullscreenZoom() {
  const chooseDown = document.querySelector('.choose-down');
  if (!chooseDown) return;

  const fullscreen = isInFullscreen();

  if (fullscreen && !chooseDown.classList.contains('fullscreen-active')) {
    chooseDown.classList.add('fullscreen-active');
  } else if (!fullscreen && chooseDown.classList.contains('fullscreen-active')) {
    chooseDown.classList.remove('fullscreen-active');
  }
}

document.addEventListener('fullscreenchange', updateFullscreenZoom);
document.addEventListener('webkitfullscreenchange', updateFullscreenZoom);
document.addEventListener('mozfullscreenchange', updateFullscreenZoom);
document.addEventListener('MSFullscreenChange', updateFullscreenZoom);

let resizeTimer;
window.addEventListener('resize', () => {
  clearTimeout(resizeTimer);
  resizeTimer = setTimeout(updateFullscreenZoom, 100);
});


window.addEventListener('load', () => {
  setTimeout(updateFullscreenZoom, 500);
});

setInterval(updateFullscreenZoom, 2000);