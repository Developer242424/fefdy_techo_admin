...
function loadQuestion() {
  ...
  // PAUSE TIMER when question loads
  pauseTimer();
  ...
  100% {
    ...
    filter: brightness(0.5);
      ...
      `);
        ...
        // WAIT 3 SECONDS, then resume timer and start tick sound
        setTimeout(() => {
          const images = $.doc.querySelectorAll(".content img");
          images.forEach(img => {
          ...
          // RESUME TIMER and START TICK SOUND
          resumeTimer();
          bgMusic.volume = 0.3;
...
function showQuestionIntro(callback) {
  ...
  document.getElementById("qm-text").textContent =
    q.question || "Get Ready!";

  const countdownEl = document.getElementById("qm-countdown");
  let count = 3;
...
// Replace the spawnWorldExplosion function with this:
...
function spawnWorldExplosion(target, doc) {
  ...
  // CONFETTI BURSTS - Different shapes and colors
  const confettiColors = ["#FF1493", "#00CED1", "#FFD700", "#00FF00", "#FF6347", "#9370DB"];
  ...
  for (let i = 0; i < 40; i++) {
    ...
    conf.animate([
      ...
      {
        transform: `translate(calc(-50% + ${Math.cos(angle) * distance}px), calc(-50% + ${Math.sin(angle) * distance}px)) scale(0) rotate(360deg)`,
        opacity: 0
      }
    ], {
      duration: duration,
    ...
    setTimeout(() => conf.remove(), duration + delay + 50);
  }

  // STAR SHAPES - Sparkly effect
  for (let i = 0; i < 25; i++) {
    const star = doc.createElement("div");
    const angle = Math.random() * Math.PI * 2;
    const distance = Math.random() * 250 + 80;
    const size = Math.random() * 8 + 100;
  ...
  // RIBBON STRIPS - Fluttery ribbons
  for (let i = 0; i < 15; i++) {
    ...
    ribbon.animate([
      ...
      {
        transform: `translate(calc(-50% + ${Math.cos(angle) * distance}px), calc(-50% + ${Math.sin(angle) * distance}px)) rotate(720deg) scaleY(0.3)`,
        opacity: 0
      }
    ], {
      duration: duration,
    ...
    setTimeout(() => ribbon.remove(), duration + delay + 50);
  }
...
function enableCorrectAnswerHandler(doc) {
  ...
  items.forEach((item) => {
    ...
    item.addEventListener("click", (e) => {
      ...
      // Get click position and call jar collection
...
function addStarToCollection(clickX, clickY) {
  ...
  const collectionRect = starCollection.getBoundingClientRect();
  const targetX = collectionRect.right - 50; 
  const targetY = collectionRect.bottom - 50; 
  ...
  flyingStar.appendChild(starImg);
  ...
  setTimeout(() => {
    // Calculate the distance to travel from current position to target
    ...
    setTimeout(() => {
      ...
      collectedStars++;
      starCounter.textContent = collectedStars;

      starCounter.style.transform = 'scale(1.3)';
      setTimeout(() => {
        starCounter.style.transition = 'transform 0.3s ease';
...
window.hideJarContainer = function () {
  ...
  collectedStars = 0;
  document.getElementById('collectedStars').innerHTML = '';
  document.getElementById('starCounter').textContent = '0';
};