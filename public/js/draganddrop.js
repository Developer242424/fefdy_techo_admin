...
putSound.onerror = () => console.error("❌ putSound failed to load");
catchSound.onerror = () => console.error("❌ catchSound failed to load");

putSound.oncanplaythrough = () => console.log("✅ putSound ready");
catchSound.oncanplaythrough = () => console.log("✅ catchSound ready");
...
function loadQuestion() {
  ...
  const cardsToDisplay =
    wrongCardIds.length > 0
      ? allCards.filter((card) => wrongCardIds.includes(card.id))
      : allCards;
  ...
  updateProgressBar(cardsToDisplay.length);
}
...
function handleDrop(e, categoryIndex) {
  ...
  console.log("🎵 About to play putSound"); 
  ...
  const maxX = dropZone.offsetWidth - 200;
  const maxY = dropZone.offsetHeight - 200;
    ...
    if (cardsInCategory.length === 0) {
      ...
      cardsInCategory.forEach((droppedCard) => {
        ...
        if (isWrong) {
          ...
          correctCategory.textContent = `Should be: ${dragDropQuestion.slice(1)[droppedCard.correctCategory].name
            }`;
...
function showRetryModal() {
  ...
  <div class="modal-overlay">
    ...
    document.getElementById("retry-zero-btn").onclick = () => {
      ...
      // Store all card IDs as wrong for next round (since all were wrong)
      wrongCardIds = allDroppedCards.map((c) => c.cardId);
      isRetryMode = true;

      // Reset for retry but KEEP time
      matchedPairs = [];
      allDroppedCards = [];
      totalScore = 0;
      loadQuestion();
    ...
    // Normal retry modal for non-zero scores
    box.innerHTML = `
    ...
    document.getElementById("retry-yes-btn").onclick = () => {
      ...
      // Store wrong card IDs for next round
      wrongCardIds = allDroppedCards
        .filter((c) => !c.isCorrect)
        .map((c) => c.cardId);

      isRetryMode = true;

      // Reset for retry but KEEP score and time
      matchedPairs = [];
      allDroppedCards = [];
      loadQuestion();
    };
...
function showSuccessModal() {
  ...
  let titleText = { "Great Job": "Great Job", "Awesome Work": "Awesome Work", "Well Done": "Well Done", "Congratulations": "Congratulations" };

  if (!modal) return;

  for (const key in titleText) {
    if (totalScore === allDroppedCards.length && key === "Great Job") {
      modalTitle.textContent = titleText[key];
      break;
    }
    else if (totalScore >= allDroppedCards.length * 0.8 && key === "Awesome Work") {
      modalTitle.textContent = titleText[key];
      break;
    }
    else if (totalScore >= allDroppedCards.length * 0.5 && key === "Well Done") {
      modalTitle.textContent = titleText[key];
      break;
    }
  ...
  modalTitle.textContent = titleText[modalTitle.textContent] || "Great Job";
...
function completeTest() {
  ...
  const wrong_answer = totalImages - totalScore;
...
function adjustScaleForIframe() {
  ...
  if (insideIframe) {
    ...
    else if (width < 900) scaleValue = 0.68;
