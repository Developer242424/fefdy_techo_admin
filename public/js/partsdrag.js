...
let hintVideoShown = false;
let isFirstPiece = true;
let inactivityTimer = null;
let heldElementTimer = null;
let lastDragTime = 0;
let isDraggingCurrently = false;

const DRAG_INACTIVITY_THRESHOLD = 8000;
const HELD_ELEMENT_THRESHOLD = 5500;
const FIRST_PIECE_DELAY = 1000;
...
function loadQuestion() {
  ...
  questionTextData = partsdrag_questions[0].question.text;
...
// ============================================================================
// DETECT DRAG START (When kid grabs a piece)
// ============================================================================
function onDragStart() {
  isDraggingCurrently = true;
  lastDragTime = Date.now();

  // Reset held element timer
  clearTimeout(heldElementTimer);


  heldElementTimer = setTimeout(() => {
    if (isDraggingCurrently) {
      console.log("Held element too long without placing");
      showAutoHint("held_too_long");
    }
  }, HELD_ELEMENT_THRESHOLD);

  resetInactivityTimer();
}

// ============================================================================
// DETECT DRAG END (When kid releases/drops a piece)
// ============================================================================
function onDragEnd_Enhanced() {
  isDraggingCurrently = false;
  clearTimeout(heldElementTimer);

  // Start watching for inactivity after they release
  resetInactivityTimer();
}

// ============================================================================
// INACTIVITY DETECTION (No dragging for 8 seconds)
// ============================================================================
function resetInactivityTimer() {
  ...
  inactivityTimer = setTimeout(() => {
    ...
    const unplacedCount = Array.from(
      document.querySelectorAll(".answer-element")
    ).filter(el => !el.dataset.placedZone).length;


    if (unplacedCount > 0 && !isDraggingCurrently) {
      console.log("8 seconds of inactivity detected");
      showAutoHint("inactivity");
    }
  }, DRAG_INACTIVITY_THRESHOLD);
}

// ============================================================================
// FIRST PIECE DEMO (Show immediately after game starts)
// ============================================================================
function showFirstPieceDemo() {
  if (hintVideoShown || !isFirstPiece) return;
...
// ============================================================================
// SHOW AUTO HINT VIDEO
// ============================================================================
function showAutoHint(reason) {
  if (hintVideoShown) return;

  stopTimer();

  const modal = document.createElement("div");
  modal.id = "hint-video-modal";
  modal.className = "hint-modal";

  const box = document.createElement("div");
  box.className = "hint-modal-box";

  let hintText = "";
  let videoSrc = "";

  switch (reason) {
    case "first_piece_demo":
      hintText = "Watch how to play!";
      videoSrc = "/partsDrag/drag-demo.mp4";
      break;
    case "inactivity":
      hintText = "Not sure what to do? Watch this!";
      videoSrc = "/partsDrag/drag-demo.mp4";
      break;
    case "held_too_long":
      hintText = "Let me show you where to drop it!";
      videoSrc = "/partsDrag/drag-demo.mp4";
      break;
  }

  box.innerHTML = `
    <h2 class="hint-title">${hintText}</h2>
    <video class="hint-video" autoplay muted loop playsinline>
      <source src="${videoSrc}" type="video/mp4">
      Your browser does not support the video tag.
    </video>
    <div class="hint-video-controls">
      <button id="hint-close-btn" class="hint-btn">Got it!</button>
    </div>
  `;
  ...
  // Replay the video
  document.getElementById("hint-replay-btn").onclick = () => {
    const video = modal.querySelector(".hint-video");
    video.currentTime = 0;
    video.play();
  };
...
function showManualHint() {
  console.log("Manual hint requested");
  hintVideoShown = false;
  showAutoHint("manual_request");
}

// ============================================================================
// ADD HINT BUTTON TO UI
// ============================================================================
function addHintButtonToUI() {
...
function onDragMove(e) {
  if (!draggedElement) return;
  draggedElement.style.left = e.clientX - offsetX * currentScale + "px";
  draggedElement.style.top = e.clientY - offsetY * currentScale + "px";
...
function onDragTouchMove(e) {
  ...
  e.preventDefault();
  const touch = e.touches[0];
  draggedElement.style.left = touch.clientX - offsetX * currentScale + "px";
  draggedElement.style.top = touch.clientY - offsetY * currentScale + "px";
...
function showSuccessModal() {
  // Clear all hint timers to prevent hint modal from appearing
  clearTimeout(inactivityTimer);
  clearTimeout(heldElementTimer);
  isDraggingCurrently = false;
